...
# --- SIDEBAR BRANDING ---
with st.sidebar:
    try: st.image("assets/logo_jcdc.png", width=150)
    except: st.write("📌 JCDC TOGO")
    st.title("Direction Générale")
    st.write("Responsable : **Sylvestre BOCCO**")
    try: st.image("assets/drapeau_togo.png", width=80)
    except: st.write("🇹🇬 Togo")
    st.markdown("---")
    niveau = st.selectbox("Échelle", ["Préfectures", "Communes"])
    path = f"data/{niveau}_Togo.shp"
    if st.button("🔄 Recommencer le Quiz"):
        for k in ["score", "nb_q", "termine"]: st.session_state[k] = 0 if k != "termine" else False
...
# --- INTERFACE ---
tab1, tab2, tab3 = st.tabs(["📖 Révision", "🎮 Défi", "🏆 Classement"])

with tab1:
    st.plotly_chart(px.choropleth_mapbox(df_gps, geojson=df_gps.geometry, locations=df_gps.index, color=col_nom, mapbox_style="carto-positron", zoom=6, center={"lat": 8.6, "lon": 1.2}, opacity=0.5, hover_name=col_nom), use_container_width=True)
...
with tab2:
    if not st.session_state.termine:
        if st.session_state.nb_q < 5:
            ...
            if st.session_state.cible is None: st.session_state.cible = random.choice(df[col_nom].unique().tolist())
            ...
            with st.form(key=f"f_{st.session_state.nb_q}"):
                ...
                if st.form_submit_button("Valider ✅"):
                    ...
                    else: st.error(f"Faux... C'était {st.session_state.cible}")
                    st.session_state.nb_q += 1; st.session_state.cible = None; time.sleep(1); st.rerun()
        else: st.session_state.termine = True; st.rerun()
    else:
        st.balloons(); st.header(f"Score : {st.session_state.score}/5")
        nom_c = st.text_input("Nom pour l'attestation et le classement :")
        if nom_c:
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🏆 Enregistrer mon Score"):
                    sauvegarder_score(nom_c, st.session_state.score, niveau); st.success("Enregistré !")
            with col2:
                if st.button("🎓 Générer mon Certificat"):
                    pdf = FPDF(orientation='L', unit='mm', format='A4'); pdf.add_page()
                    pdf.set_draw_color(0, 106, 77); pdf.rect(5, 5, 287, 200); pdf.set_draw_color(255, 206, 0); pdf.rect(10, 10, 277, 190)
                    try: pdf.image("assets/logo_jcdc.png", 20, 20, 35); pdf.image("assets/drapeau_togo.png", 240, 20, 35)
                    except: pass
                    pdf.set_y(70); pdf.set_font("Arial", 'B', 30); pdf.set_text_color(0, 106, 77); pdf.cell(0, 20, "CERTIFICAT DE RÉUSSITE", ln=True, align='C')
                    pdf.set_font("Arial", 'B', 25); pdf.set_text_color(216, 12, 18); pdf.cell(0, 20, nom_c.upper(), ln=True, align='C')
                    pdf.set_font("Arial", '', 15); pdf.set_text_color(0,0,0); pdf.cell(0, 10, f"Score : {st.session_state.score}/5 sur la carte des {niveau}", ln=True, align='C')
                    pdf.output("certif.pdf")
                    with open("certif.pdf", "rb") as f: st.download_button("⬇️ Télécharger", f, "Certificat_JCDC.pdf")

with tab3:
    st.subheader("🥇 Top 10 des Experts")
    if os.path.exists("leaderboard.csv"):
        st.table(pd.read_csv("leaderboard.csv").sort_values(by="Score", ascending=False).head(10))
    else: st.info("Aucun score enregistré.")